...
We are calculating HVG in the script, but using a threshold of 0.25 for the dispersion.      
...
library(paste0("org.",params$sp,".eg.db"),character.only = T)
...
if (my.subset) {
  p.Wdata = Seurat::SubsetData(p.Wdata, idents=my.subset)
}

# If no variable genes are provided
nonex = which(apply(p.Wdata@assays$RNA@counts, 1, function(x) length(which(x >0))) < 10)

if (my.vargenes == F) {
  # First get rid of non-expressed genes
  p.Wdata = Seurat::SubsetData(p.Wdata, features = rownames(p.Wdata@assays$RNA)[-nonex])
...
```{r soft-thresholding power, message=FALSE}
...
while(nonsig != 0) {
  ...
  for (i in x) {
    for (j in y) {
      sink("aux")
      z=table(WGCNA::cutreeDynamic(dendro = geneTree,  method="tree", minClusterSize = i, deepSplit = T, cutHeight = j, verbose = 0))
      sink(NULL)
  ...
  dynamicMods = WGCNA::cutreeDynamic(dendro = geneTree,  method="tree", minClusterSize = my.Clsize, deepSplit = T, cutHeight = my.height)
...
raw.datExpr = p.Wdata@assays$RNA@scale.data[colnames(datExpr),]
...
For single cells
We can see what are the expression levels of our co-expression modules. If we provided a seurat object. We look, in this case, at a tSNE
...
for (i in 1:length(yy)) {
  ...
  xx[[i]] = ggplot2::ggplot(toplot[order(p.MEList$averageExpr[,i]),],
    ...
    ggplot2::geom_point(aes_string(color=p.MEList$averageExpr[order(p.MEList$averageExpr[,i]),i]), size=2) +
...
WGCNA_data[["TOM"]]= TOM
WGCNA_data[["adjacency"]]= adjacency

saveRDS(WGCNA_data, file = paste0(params$dir,"/data/",project_name,"_WGCNA_data.rds"))
...
my.dirname = paste0(params$dir,"/data/",project_name,"WGCNA_networks/")
...
for (i in 1:length(xx)) {
  ...
  mynet = network::set.vertex.attribute(mynet, "membership", mynodes[network.vertex.names(mynet),"membership"])
...
my.ENS2EG = get(paste0("org.",my.sp,".egENSEMBL2EG"))
...
for(i in 1:length(yy)){ #as many clusters as we have
  ...
  ex = sapply(dewg[[i]], function(x) exists(x, my.ENS2EG)) #Which genes have an ENTREZ?
...
ggplot2::ggplot(g.plot, ggplot2::aes(x=Term, y=-log10(P.DE), fill=cluster)) +
    ggplot2::geom_point(aes(shape = cluster), size = 3) +
...
for(i in 1:length(xx)){
  ...
  myplot = ggplot2::ggplot(toplot[order(p.MEList$averageExpr[,i]),],
                   ggplot2::aes_string(x=colnames(toplot)[1], y=colnames(toplot)[2])) +
      ggplot2::geom_point(aes_string(color=p.MEList$averageExpr[order(p.MEList$averageExpr[,i]),i])) +
      ggplot2::scale_size(range = c(1, 1)) +
      ggplot2::theme_void() +
      ggplot2::scale_colour_gradientn(colours = c("gray90", "gray90", yy[i], yy[i])) +
      ggplot2::labs(colour=levels(as.factor(dynamicColors))[i])
  ...
  mynetplot = GGally::ggnet2(mynet,
                         ...
                         layout.par = list(repulse.rad=network.size(mynet)^1.1,
                                           area=network.size(mynet)^2.3), # Give space in the middle
                         node.size = get.vertex.attribute(mynet,'membership'), max_size = 22, #The size of the nodes
                         node.color = levels(as.factor(dynamicColors))[i], # The color of the module
                         edge.size = "weight",
                         edge.color = "black",
                         edge.alpha = get.edge.attribute(mynet,'weight01'),
                         ) +
    ggplot2::theme(legend.position="none") +
    ggplot2::geom_label(ggplot2::aes(label=gnames[network.vertex.names(mynet),2]),
